...
const createConversation = async (req, res) => {
  try {
    ...
    const validMembers = await User.find({ _id: { $in: allMembers } }).select(
      "_id"
    );
...
const findPrivateConversation = async (req, res) => {
  try {
    ...
    // Let Mongoose handle the ObjectId conversion
    ...
    console.error('Error in findPrivateConversation:', error);
...
const searchConversations = async (req, res) => {
  try {
    ...
    const memberSearchConversations = await Conversation.find({
      members: userId,
    })
      .populate({
        path: "members",
    ...
    const filteredMemberConversations = memberSearchConversations.filter(
      (conv) => conv.members && conv.members.length > 0
    );
    ...
    filteredMemberConversations.forEach((conv) => {
      if (
        !allConversations.find(
          (existing) => existing._id.toString() === conv._id.toString()
        )
      ) {
